---
id: technical-verification-walkthrough
...
Anyway for now we continue with our metadata.json. To be able to compile we first need to make sure we have everything we need. That is, all the source files outlined in the metadata.json. The JSON file has a `sources` field that looks like this
...
What do we do with this information and all other `.sol` files the user gave us?
...
We are only interested in the `compilationTarget` (e.g. `contracts/1_Storage.sol` and `Storage`) and not all other contracts provided. The two fields are of our interest are `evm.bytecode.object` and `evm.deployedBytecode.object`. 
...
We talked about the onchain and recompiled bytecodes. In addition this, there are also two associated bytecode types to a contract we can perform the verification on. 
...
If we got a match, it means we can start from recompiled bytecodes, apply each runtime or creation transformations on top of the respective recompiled bytecodes, and obtain the onchain bytecodes. It also means that the provided source code and metadata can be associated with the contract at this chainId and address and we mark this contract as verified.
...
One nice feature of the EOF is it separates code from data and gives structure to code. This makes us verifiers' lives much easier compared to the completely unstructured legacy EVM code. A current example can be seen in our post [Finding Auxdatas in the Bytecode](/blog/finding-auxdatas-in-bytecode/). However as of now, the metadata is still not fully isolated from the contract code as there is no separate section for that information. Current compiler implementations put the metadata in the `data_section` of EOF and this unfortunately does affect the contract's code. That's why we are proposing a separate `metadata_section` in the EVM Object Format that cannot be reached by the EVM and any changes to that will not affect the contract's code. If this is implemented, verifiers can completely ignore the metadata parts of a container, and won't have to look for workarounds.
