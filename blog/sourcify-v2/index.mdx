...
The changes **do not affect the [Sourcify Server API](https://docs.sourcify.dev/docs/api/) in a non-backwards compatible way**. If you are using the Sourcify API you don't need to worry. However are some non-breaking additions detailed below.
...
(_In the server API, find the field [`contextVariables`](https://docs.sourcify.dev/docs/api/server/verify/)_)
...
passing the `msgSender` variable allows `lib-sourcify` to assign this variable inside the bytecode, and be able to verify, because it's an [immutable variable](https://docs.sourcify.dev/docs/immutables/). If there are other potential variables that are used as a value to immutables in contracts, please let us know.
...
(_In the server API, find the field [`creatorTxHash`](https://docs.sourcify.dev/docs/api/server/verify/)_)
