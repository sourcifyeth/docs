---
id: dataset-download
slug: /repository/download-dataset/
...
When importing the dataset into a postgres database, you have to make sure to follow a specific order of tables to satisfy foreign key constraints. Have a look at the [schema](/docs/repository/sourcify-database/#schema) for the dependencies between tables.

The recommended order is the following: 1. `code`, 2. `signatures`, 3. `sources`, 4. `compiled_contracts`, 5. `contracts`, 6. `compiled_contracts_signatures` 7. `compiled_contracts_sources`, 8. `contract_deployments`, 9. `verified_contracts`, 10. `sourcify_matches`.
