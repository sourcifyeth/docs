...
This page describes the process of migrating all the contracts from the fs based repository to a new storage structure.
:::
...
We cannot move completely to a database dropping the fs because we want to keep serving metadata files and sources over IPFS. So the new storage structure includes:
...
- **Database**: a PostgreSQL database based on [the schema of the Alliance Database](https://github.com/verifier-alliance/database-specs) customized to better fit Sourcify. In particular the Alliance Database doesn't allow storing contracts without creation bytecode and it doesn't explicitly support the concept of [full vs partial verification](notion://www.notion.so/docs/full-vs-partial-match/).
...
1. **Enable RepositoryV2 and Database in `sourcify-server`**, `sourcify-server` will store the new matches into RepositoryV2 and the Database (including `sourcify_sync`). `sourcify-server` will continue writing into *current_repositoryV1*.
...
4. **Start using the new_repositoryV1.** After the migration is completed, on the dedicated  `sourcify-server-migration` instance, the new version of repositoryV1 will be produced (*new_repositoryV1*). Before using *new_repositoryV1* in `sourcify-server` all the contracts stored in *current_repositoryV1* after starting the migration script needs to be moved from *current_repositoryV1* to *new_repositoryV2*. 
...
- **b. Migrate the database schema**
    ...
    - I. Clone the Sourcify monorepo and `npm install`
    - II. `cd services/database`
    - III. Edit `database.json` to configure the connection to the PostgreSQL database
    - IV. `npm run migrate:up -- --env=YOUR_ENV` Instead of `YOUR_ENV` put the key used to define the database connection in `database.json` in the previous step.
- **c. Enable RepositoryV2**
    - Make sure that in `services/server/src/config` there is the `repositoryV2` configuration. Set the path accordingly.
...
- **b. Wait**
    ...
    - Wait until the scripts exits with `successfuly imported from “:repo_path” :number_imported_contracts contracts.`

### 3. Start the migration instance and the migration script

- a. Start a dedicated `sourcify-server-migration` instance
    - I. Start another `sourcify-server` instance dedicated to the migration process 
    ⚠ You need to checkout **6cdcdce8f186cdf3c90438393fee1efcbe272622**  or `git checkout tags/sourcify-server@1.6.0`
    ...
    - III. ⚠ In `sourcify-server-migration` change the path of repositoryV1 to a new path: this is the *new_repositoryV1*. Don’t write in *current_repositoryV1*!
- **b. Find unsupported chains**
    - I. From `services/server/src/sourcify-chains-default.json`  or the the `sourcify-chains` file that you are using find all the chainIds of unsupported chains.
...
- **c. Start the synchronization script**
    ...
    - III. `npm run sourcify:database sync URL_TO_SOURCIFY_MIGRATION_INSTANCE /data/repository/contracts/ --  -c LIST_OF_UNSUPPORTED_CHAINS  --limit 5`
        ...
        - You can configure `—limit 5`: limit is the number of concurrent verification that are going to be sent to the `sourcify-server-migration` for each chain. If you set 5, 5 requests are going to be sent for each chain.
- **d. Wait and monitor**
    ...
    - The script will output a verification per second insight. The verification speed is a good compromise between the amount of concurrent requests sent for each chain and the amount of requests the rpc can handle. If you see that you are receiving a lot of rpc errors decrease the number of concurrent verifications, if the verifications per seconds are too low, increase the number of concurrent verifications. You can set the number of concurrent verifications using the `--limit` parameter.

### **4. Start using the new deprecated repositoryV1**

- While the migration script was running on `sourcify-server-migration`, new matches got saved in Database, RepositoryV2 and *current_repositoryV1* coming from `sourcify-server`.
    ...
    - A procedure is needed to transfer all the contracts in *current_repositoryV1* that were saved after RepositoryV2 and Database were enabled on `sourcify-server`.
        ...
        - Extract all the contracts described in the previous point from the database in a `rows.txt` file. For each row output the path in the repository. You can use the following query:
...
- Switch the RepositoryV1 config to the *new_repositoryV1*’s path.
...
- IPFS instance is pinning the repoV2
- and
-