...
1. Fork the repository. Branch out from the **staging** branch with name `add-chain-{chainId-1}-{chainId-2}` (e.g. for Ethereum Mainnet and goerli `add-chain-1-5`). This branch name format is required for automated testing. You can also add more than one chains in one pull request, granted you added tests for each.
...
1. To test locally run `NEW_CHAIN_ID={chainId1},{chainId-2} npx mocha test/chains/chain-tests.js --exit`
