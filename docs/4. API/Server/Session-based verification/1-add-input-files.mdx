---
id: session-add-input-files
title: Add input files
slug: /api/server/session/add-input-files
---
...
- If possible, runs the process of verification, as described in [Verify](/docs/api/server/session/verify-validated).

<V2Note />

**URL** : `/session/input-files`
...
```json
{
  "error": "Too much session memory used. Delete some files or clear the session."
}
```
